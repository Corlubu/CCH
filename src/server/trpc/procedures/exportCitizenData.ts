...
export const exportCitizenData = baseProcedure
  .input(
    z.object({
      authToken: z.string(),
      role: z.enum(["CITIZEN"]).optional(), // For consistency with getAllUsers, though we'll always export citizens
    })
  )
  .query(async ({ input }) => {
    ...
    if (decoded.role !== "ADMIN" && decoded.role !== "STAFF") {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Only administrators and staff can export citizen data",
      });
    }
    ...
    return {
      csv: csvContent,
      filename: `citizen-data-export-${new Date().toISOString().split("T")[0]}.csv`,
