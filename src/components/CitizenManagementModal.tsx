...
export function CitizenManagementModal({
  isOpen,
  onClose,
  mode,
  authToken,
  citizen,
}: CitizenManagementModalProps) {
  const trpc = useTRPC();
  ...
  const {
    ...
    defaultValues: mode === "edit" && citizen ? {
  ...
  const createMutation = useMutation(
    trpc.createCitizen.mutationOptions({
      onSuccess: () => {
  ...
  const updateMutation = useMutation(
    trpc.updateCitizen.mutationOptions({
      onSuccess: () => {
      ...
      <div className="w-full max-w-2xl rounded-xl bg-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-gray-900">
            {mode === "create" ? "Create New Citizen" : "Edit Citizen"}
          </h2>
          <button
            onClick={onClose}
